...
def experiment(seed):
    ...
    Square_LIN_in = myfunc.squaredError(w_LIN,x_train_data,y_train_label) # Q9
    Zero_One_LIN_in = myfunc.zeroOneError(w_LIN,x_train_data,y_train_label) # Q10
    ...
    # LOGREG with outlier
    w_LOGREG_outlier = myfunc.logisticRegression(x_outlier_data,y_outlier_label,ITERATION,ETA)
    Zero_One_LOGREG_outlier_out = myfunc.zeroOneError(w_LOGREG_outlier,x_test_data,y_test_label) # Q12
    return (Square_LIN_in,Zero_One_LIN_in,Zero_One_LIN_out,Zero_One_LOGREG_out,Zero_One_LIN_outlier_out,Zero_One_LOGREG_outlier_out)

if __name__ == "__main__":
    exp_times = 128
    pool = mp.Pool(mp.cpu_count())
    results = pool.map(experiment,range(exp_times))
    np.savetxt("exp_result.txt",results,header="Square_LIN_in,Zero_One_LIN_in,Zero_One_LIN_out,Zero_One_LOGREG_out,Zero_One_LIN_outlier_out,Zero_One_LOGREG_outlier_out")
